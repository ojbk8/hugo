...
func TestLanguageFs(t *testing.T) {
	...
	enFs := FileMeta{"lang": "en", "fs": afero.NewMemMapFs()}
	svFs := FileMeta{"lang": "sv", "fs": afero.NewMemMapFs()}

	for _, fs := range []FileMeta{enFs, svFs} {
		assert.NoError(afero.WriteFile(fs.Fs(), filepath.FromSlash("blog/a.txt"), []byte("abc"), 0777))

		for _, fs2 := range []FileMeta{enFs, svFs} {
